...
namespace otr {
   ...
   export const send = async function (ws, contacts, username, longKey, message) {
      console.log('SENDING', contacts, message);
      let to;
      ...
      console.log(to, username, longKey);
      return message;
   }
   export const recieve = async function (ws, contacts, username, longKey, message) {
      console.log('RECIEVING', contacts, message);
      let from;
      for (let i = 0; i < contacts.length; i++) {
         const contact = contacts[i];
      ...
      console.log(from, username, longKey);
      return message;
   }
   ...
   export namespace test {
      ...
      const testAll = async function () {
         ...
         await aliceConvo.send(network); // ake1
         console.log(aliceConvo, network);
         await bobConvo.recieve(network); // ake2
         console.log(network);
         await aliceConvo.recieve(network); // ake3        
         console.log(network);
         await bobConvo.recieve(network); // ake4
         console.log(network);
         await aliceConvo.recieve(network); // ake5        
         console.log(network);
         await bobConvo.recieve(network); // ed1
         console.log(network);
